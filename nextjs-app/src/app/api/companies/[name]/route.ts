...
export async function DELETE(req: Request, { params }: { params: Promise<{ name: string }> }) {
  try {
    ...
    await storage.deleteCompanyTasks(decodedName);
    return NextResponse.json({ success: true, message: `All tasks for ${decodedName} deleted successfully.` });
  } catch (err) {
    ...
    console.error("Error deleting company tasks:", err);
    return NextResponse.json(
