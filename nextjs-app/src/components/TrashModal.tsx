...
export function TrashModal() {
  ...
  const confirmPermanentDelete = () => {
    if (!companyToDelete || !secretCode) return;
    
    permanentDeleteMutation.mutate({ companyName: companyToDelete, secretCode }, {
      onSuccess: () => {
        toast({ title: "Deleted", description: `${companyToDelete} permanently deleted.` });
    ...
    <>
      ...
      <AlertDialog open={!!companyToDelete} onOpenChange={(open) => !open && setCompanyToDelete(null)}>
        <AlertDialogContent className="rounded-2xl border-destructive/20 shadow-2xl">
          <AlertDialogHeader>
            <div className="bg-destructive/10 w-fit p-3 rounded-2xl mb-2">
              <Trash2 className="w-6 h-6 text-destructive" />
            </div>
            <AlertDialogTitle className="text-2xl font-display">Permanent Deletion</AlertDialogTitle>
            <AlertDialogDescription className="text-base">
              Are you sure you want to permanently delete <span className="font-bold text-foreground">"{companyToDelete}"</span> and all its associated tasks? This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
